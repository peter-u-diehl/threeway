...
class RelationalNetwork:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.dataPath + 'delays/'):
            os.makedirs(self.dataPath + 'delays/')
    ...
    def saveDelays(self, ending=''):
        ...
        for connName in self.connections:
            if connName == 'ZeCi':
                connMatrix = self.connections[connName].delay[:]
                if connName[1]=='e':
                    nSrc = self.nE
                else:
                    nSrc = self.nI
...
#             spikeMonitors[name+'e'] = SpikeMonitor(inputGroups[name+'e'])
    ...
    def createInputConnections(self):
        ...
        for name in self.inputConnectionNames:
            ...
            for connType in self.inputConns:
                ...
                weightMatrix, weightList = self.getMatrixFromFile(self.weightPath+connName+'.npy')
                ...
                delayMatrix, delayList = self.getMatrixFromFile(self.delayPath+connName+'.npy')
    ...
    def run(self):
        ...
        #------------------------------------------------------------------------------ 
        previousSpikeCount = np.zeros(self.nE)
        ...
        for j in xrange(int(self.numExamples)):            
            ...
            for i,name in enumerate(self.inputPopulationNames):
                if name == 'X':
                    ...
                    self.resultMonitor[j,1] = self.popValues[i]
                else:
                    if self.testMode:
                        rates = np.ones(self.nE)  * 0
                    elif name == 'Y':
            ...
            currentSpikeCount = np.asarray(self.spikeCounters['Ce'].count[:]) - previousSpikeCount
        #     print currentSpikeCount,  np.asarray(spikeCounters['Ce'].count[:]), previousSpikeCount
            previousSpikeCount = np.copy(self.spikeCounters['Ce'].count[:])
            self.resultMonitor[j,0] = self.computePopVector(currentSpikeCount)
            difference = np.abs((self.resultMonitor[j,1]**2)%1. - self.resultMonitor[j,0])
            if difference > 0.5:
                difference = 1-difference
            print 'Pop. activity: ', self.resultMonitor[j,0], ', Desired activity: ', (self.resultMonitor[j,1]**2)%1., ', Difference: ', difference
