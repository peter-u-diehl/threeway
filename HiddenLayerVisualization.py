...
def plotWeights(dataPath='./', ending=''):
    ...
    WEIGHTS_TO_H = True
    ANNOTATE_DOTS = False
    print 'visualize hidden weights '
    #===============================================================================
    # load data
    #===============================================================================
    if WEIGHTS_FROM_H:
    ...
    if WEIGHTS_FROM_H:
        error1 = np.abs(np.mod(popVecs[:,0] + popVecs[:,1], nTgt) - popVecs[:,2])/nTgt
    ...
    if WEIGHTS_TO_H:
        error2 = np.abs(np.mod(popVecs[:,3] + popVecs[:,4], nTgt) - popVecs[:,5])/nTgt
