    ...
    for i,name in enumerate(net.inputPopulationNames):
        if name == 'X':
            ...
            if net.testMode:
                ...
                net.popValues[j,i] = (net.popValues[j,0]*2) % 1.
                rates = net.createTopoInput(net.nE, net.popValues[j,i])
            elif name == 'Z':
                net.popValues[j,i] = (net.popValues[j,0]**2) % 1.
                rates = net.createTopoInput(net.nE, net.popValues[j,i])
...
def evaluate(dataPath, ending='1000', noise='', start_time=0, nE=1600):
    print 'ending', ending
    end_time = int(ending)
